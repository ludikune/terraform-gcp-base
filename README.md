...
Start by setting the corresponding values in bootstrap/state/terraform.tfvars. Then use the bootstrap/state config as usual (terraform init/plan/apply) to create the bucket which will be used as remote backend for the terraform state of all environments.
